...
# ERROR:data_fetcher:Error saving to cache: Object of type DataFrame is not JSON serializable
# ! json cannot handle series/dataframe/ index/ timestamp/ datetime
# it recursively converts pandas objects to a serializable format
...
class DataFetcher:
   ...
   # FIX!!!!: instead of stating which table it is , see which table has the correct columns 
    ...
    # Field: 
    # historial: get historical data 
    # info: address/ city. state ....
    # financials keys: income statement, balance sheet, cash flow
                ...
                for symbol in batch:
                    try:
                        ...
                        if len(batch) == 1:
                            ...
                            # If we have multiple symbols, data is a DataFrame with columns for each symbol 
                            symbol_data = data[symbol]
                        
                        # no daa for the symbol
                        ...
                        # Get additional info ( such as sector, industry, etc) stores in info 
                        ticker = yf.Ticker(symbol)
                        result[symbol]['info'] = ticker.info
                        
                        # Get other statments statement if available 
                        try:
                            ...
                            logger.warning(f"Could not fetch financial statement for {symbol}")
...
# Fix: i think i should get rid of this... or try another API? 
# --------------------------------- only if has set alpha vantage key and it needs premiuns  -------------------
    ...
    def save_to_cache(self, data, filename):
        ...
        try:
            ...
            with open(filepath, 'w') as f:
                json.dump(data, f) # try save pandas data frame to a json file 
