...
import io # Needed for parsing string to DataFrame
import sys
import os

# Add the project root to the Python path
# This ensures that modules within the 'app' directory can be imported
current_dir = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.dirname(os.path.dirname(current_dir)) # Go up two levels from app/interface.py to get to project root
...
def main():
    ...
    selection = st.sidebar.radio("Go to", ["Technical Analysis", "Fundamental Analysis", "Stock Screener"])

    if selection == "Technical Analysis":
        show_technical_analysis_section()
    elif selection == "Fundamental Analysis":
        show_fundamental_analysis_section()
    elif selection == "Stock Screener":
...
def show_fundamental_analysis_section():
    ...
    if st.button("Fetch Fundamental Data", key="fetch_fund_data_btn"):
        with st.spinner(f"Fetching fundamental data for {symbol}..."):
            data = st.session_state.fetcher.fetch_yfinance_data(symbol, period="1d", interval="1d") # Only fetch info, historical not needed here
